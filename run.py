...
def main(config: Dict):
    ...
    parser.add_argument('--inference', default=config["inference"], type=bool)
    ...
    parser.add_argument('--resume', default=config["resume"], type=bool)
    ...
    if not args.inference:
        ...
        model = Model.load_from_checkpoint(latest_version_path)
        ...
        output_file_name = '_'.join(latest_version_path.stem.split("_")[:-2]) + "_" + datetime.today().strftime("%Y%m%d_%H%M%S") + ".csv" # add prediction time
